...
export function SeoContent({ year, strategies = [] }: { year: number; strategies?: VacationOpportunity[] }) {
  ...
  const faq: FaqItem[] = [
    ...
    { question: `Jak znaleźć przynajmniej dwa tygodnie wolnego w ${year}?`, answer: <><p>Szukaj dłuższych bloków obejmujących dwa weekendy i święta. W strategii urlopowej ustaw minimum 14 dni wolnego oraz tyle dni urlopu, ile możesz przeznaczyć na wyjazd.</p>{longBreak && <><p>Przykład z kalendarza {year}:</p><OpportunityAnswer strategy={longBreak} /></>}<p>Bez świąt 14 kolejnych dni to 10 dni roboczych przy pracy od poniedziałku do piątku. <a href="/kalkulator-urlopu/">Sprawdź własny termin w planerze urlopu →</a></p></> },
