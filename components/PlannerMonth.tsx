...
export function PlannerMonth({ month, activeYear, plan, leave, donated, breakDates, blockedDonations, tool, ready, planningDate, onSelect, onSwitchYear, onClose }: {
  month: MonthData;
  activeYear: number;
  plan: PersonalPlan;
  leave: Set<string>;
  ...
  return <section className={`plan-month ${adjacent ? 'plan-month-adjacent' : ''}`} id={`plan-month-${month.year}-${month.monthIndex}`} aria-label={`${month.name} ${month.year}`} tabIndex={-1}>
    ...
    <h3>{month.name}{(adjacent || month.monthIndex === 0 || month.monthIndex === 11) && <span> {month.year}</span>}</h3>
    ...
    <div className="plan-days">{month.weeks.flat().map((day, i) => {
      ...
      return <button key={key} type="button" data-date={key} className={`plan-day ${day.date.getDay() === 0 || day.date.getDay() === 6 ? 'is-weekend' : ''} ${holiday ? 'is-holiday' : ''} ${inBreak ? 'is-break' : ''} ${breakStart ? 'break-start' : ''} ${breakEnd ? 'break-end' : ''} ${selected && !donation ? 'is-leave' : ''} ${donation ? 'is-donation' : ''} ${schoolClass} ${blocked ? 'is-blocked' : ''} ${key === planningDate ? 'is-today' : ''}`} aria-label={label} title={blocked || label} aria-disabled={blocked ? true : undefined} aria-pressed={tool === 'leave' ? selected : !!donationStart} disabled={!ready || (tool === 'leave' && (!working || (donation && !selected)))} onClick={() => onSelect(key)}><span className="plan-day-number">{day.date.getDate()}</span>{donation && <DonationIcon className="plan-donation-mark" />}</button>;
    })}</div>
