...
test('calendar bridges merge, persist, reset and keep annual budgets separate', async ({ page }) => {
  ...
  await expect(page.locator('.plan-budget-values strong')).toHaveAttribute('aria-label', /^Wybrano 2 dni urlopu/);
  await expect(page.locator('.planner-balance-total')).toHaveText('6');
  await page.getByRole('button', { name: 'Cały rok', exact: true }).click();
  await expect(page.locator('.planner-break-row')).toHaveCount(1);
...
test('legacy strategy links still save dates, preserve the existing plan and survive reload', async ({ page }) => {
  await page.goto(route);
  ...
  const dates = analyzeVacationStrategies(2026)[0].vacationDays.map(formatDateKey);
  const target = plannerHref(Number(dates[0].slice(0, 4)), dates);
...
test('donation release and school overlays do not consume leave', async ({ page }) => {
  ...
  await expect(page.locator('.plan-budget-values strong')).toHaveAttribute('aria-label', /^Wybrano 0 dni urlopu/);
  await expect(page.locator('.planner-balance-total')).toHaveText('4');
  ...
  await page.goto(route);
  await expect(donationLegend).toHaveCount(0);
  await expect(page.locator('.planner-balance-total')).toHaveText('0');
...
test('changing profile cannot bypass rolling annual limits', async ({ page }) => {
  ...
  await expect(page.locator('.donor-profile .donor-error')).toContainText('limit 4');
});
...
test('expanded months share a cross-year plan, split budgets and reopen saved dates', async ({ page }) => {
  ...
  await expect(page.locator('.plan-budget-values strong')).toHaveAttribute('aria-label', /^Wybrano 0 dni urlopu/);
  await expect(page.getByRole('navigation', { name: 'Urlop w sąsiednich latach' })).toContainText('2 dni urlopu');
  ...
  await expect(page.locator('.plan-budget-values strong')).toHaveAttribute('aria-label', /^Wybrano 4 dni urlopu/);
  await expect(page.locator('.planner-balance-total')).toHaveText('14');
  ...
  await expect(page.locator('.plan-budget-values strong')).toHaveAttribute('aria-label', /^Wybrano 2 dni urlopu/);
  await expect(page.getByRole('navigation', { name: 'Urlop w sąsiednich latach' })).toContainText('3 dni urlopu');
  await page.locator('[data-date="2026-12-31"]').click();
  await expect(page.locator('.planner-balance-total')).toHaveText('14');
...
test('adjacent months enforce donation limits and load the school dates of their own year', async ({ page }) => {
  ...
  await page.goto(route);
  await expect(page.locator('.planner-calendar-balance')).toContainText('1 dni roboczych za donacje');
  await expect(page.getByRole('navigation', { name: 'Urlop w sąsiednich latach' })).toContainText('2 dni za donacje');
  await page.getByLabel('Tryb uczniowski').check();
