...
function PageFooter({ buildYear }: { buildYear: number }) {
  return <footer className="mt-12 border-t border-neutral-200 pt-8 text-sm text-neutral-600">
    <nav aria-label="Nawigacja stopki" className="flex flex-wrap gap-x-5 gap-y-3"><a href="/">NieRobie.pl</a><a href="/kalkulator-urlopu/">Planer urlopu</a>{indexedYears(buildYear).map(year => <a href={yearPath(year)} key={year}>Kalendarz {year}</a>)}</nav>
