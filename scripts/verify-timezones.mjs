...
const source = `import { render } from './.ssr/entry-server.js'; import { createHash } from 'node:crypto'; console.log(createHash('sha256').update(['/2027/', '/kalkulator-urlopu/', '/planer-krwiodawcy/'].map(path => render({path,buildYear:2026}).html).join('')).digest('hex'));`;
